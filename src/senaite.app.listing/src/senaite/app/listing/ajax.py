...
logger = logging.getLogger("senaite.sample.view")
...
class AjaxListingView(BrowserView):
    ...
    def ajax_set_fields(self):
        ...
        # Get the HTTP POST JSON Payload
        payload = self.get_json()
        # print("[ajax_set_fields] payload = ", payload)
        ...
        for uid, data in save_queue.iteritems():
            # print("[ajax_set_fields] uid =", uid, "data =", data)
            # get the object
            obj = api.get_object_by_uid(uid)

            # update the fields
            for name, value in data.iteritems():
                if (isinstance(value, dict) and "status" in value and "values" in value):
                    print("[ajax_set_fields] field", name, "is TieredMultiValue ->", value)
                    value = json.dump(value, ensure_ascii=False)
