...
#    @api_url = getData(@root_el, "api_url")
#    @columns = JSON.parse(getData(@root_el, "columns") or "[]")
#    @form_id = getData(@root_el, "form_id")
#    @listing_identifier = getData(@root_el, "listing_identifier")
#    @pagesize = parseInt(getData(@root_el, "pagesize") or "0")
#    @review_states = @parse_json(getData(@root_el, "review_states"))
#    @default_review_state = getData(@root_el, "default_review_state") or "default"
  ...
  getRequestOptions: ->
    ...
    extra = @getExtraPayloadFromForm() or {}
    payload = $.extend(true,{},extra,options)

    console.debug("[Listing] extra keys=",Object.keys(extra))
#    console.debug("Request Options=", options)
    console.debug("[Listing] payload=",payload)
    return payload

  getExtraPayloadFromForm: ->
    ...
    $(form).find(':input[name]').each (i, el) ->
      ...
      # checkbox 取 checked，否则取 value
      if el.type is 'checkbox'
        extra[name] = if el.checked then (el.value or 'on') else ''
      else
        extra[name] = el.value
    extra
  ...
  saveEditableField: (uid, name, value, item) ->
    ...
    ###
     * Merge data from TieredMultiValue buffer into ajax_save_queue
     * Returns true if something was merged.
    ###
  merge_tmv_buffer_into_queue: ->
    buf = window.__tmv_buffer
    return false unless buf? and Object.keys(buf).length

    queue = @state.ajax_save_queue
    changed = false

    for uid, fields of buf
      queue[uid] ?= {}
      for name, payload of fields
        # TMV 组织结构: {status: "detected"/"undetected", values: [..6个..]}
        # 后端只收一个字段值，因此这里统一转成字符串
        try
          queue[uid][name] = JSON.stringify(payload)
        catch e
          queue[uid][name] = JSON.stringify(
            status: payload?.status or "undetected"
            values: payload?.values or ["","","","",0,0]
          )
        changed = true
  ...
  # 兼容读取 data-xxx /data_xxx / dataset驼峰

  getData = (el, key) ->
    return null unless el?

    # key like "form_id" or "api_url"
    camel = key.replace /_([a-z])/g, (m, p1) -> p1.toUpperCase()
    dash  = key.replace /_/g, "-"

    el.dataset?[key] or el.dataset?[camel] or
    el.getAttribute?("data_#{key}") or el.getAttribute?("data-#{dash}") or null
