...
schema = Person.schema.copy() + atapi.Schema((
    ...
    UIDReferenceField(
        "AllowedServices",
        multiValued=True,  # 允许多选
        allowed_types=("AnalysisService",),  # 只能选实验服务
        relationship="labcontact_allowed_services",  # 反向引用键（可自定义）
        mode="rw",
        widget=ReferenceWidget(
            label=_("Allowed Analysis Services", default="Select Related Services"),
            description=_(
                "Select the analysis services this contact is allowed to access"
            ),
            ...
            catalog=SETUP_CATALOG,  # setup 目录下搜索服务
            query={
                "is_active": True,
                "sort_on": "sortable_title",
                "sort_order": "ascending",
    ...
    StringField(
        ...
        required=False,
        mode="rw",
        vocabulary="_get_users_vocabulary",
        widget=SelectionWidget(
            format="select",
            label=_("Linked System User"),
            description=_("Select the system user to link to this LabContact."),
        ),
    ),
...
class LabContact(Contact):
    ...
    def _get_users_vocabulary(self):
        """返回所有系统用户（Plone用户）组成的下拉列表"""
        mtool = getToolByName(self, "portal_membership")
        members = mtool.listMembers()

        # 生成 (id, id) 形式的用户列表
        items = [(member.getId(), member.getId()) for member in members]
        return api.to_display_list(items, sort_by="value", allow_empty=True)
