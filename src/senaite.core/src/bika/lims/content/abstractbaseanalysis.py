...
Method = UIDReferenceField(
    ...
    widget=SelectionWidget(
        format="checkbox",  # 把select修改成checkbox
...
class AbstractBaseAnalysis(BaseContent):  # TODO BaseContent?  is really needed?
    ...
    #         return method.Title()
    def getMethodTitle(self):
        """Get titles of method(s), supports both single and multi-valued"""
        raw_value = self.getMethod()

        if not raw_value:
            return ""

        try:
            # 多值情况：list/tuple of UIDs or objects
            ...
            logger.error("[getMethodTitle] 获取失败: %s", e)
    ...
    #     return method
    def getRawMethod(self):
        """Returns the UID(s) of the assigned method(s)

        NOTE: This is the default accessor of the `Method` schema field
        and needed for the selection widget to render the selected value(s)
        properly in _view_ mode.

        :returns: list of Method UIDs or None
        """
        field = self.getField("Method")
        raw = field.getRaw(self)
        if not raw:
            return []
    ...
    def getMethodURL(self):
        ...
        if not isinstance(methods, (list, tuple)):
            methods = [methods]
            return [m.absolute_url_path() for m in methods if m]
