...
def create_analysisrequest(client, request, values, analyses=None,
    ...
    # 手动写入自定义字段值
    for field_name in [
        "PatientAlias", "name", "Gender", "Age", "InitialScreeningDate",
        "SampleVolume", "SampleTypeCN", "HLA_A0201_Result",
        "SamplingNote", "Complications", "MedicalHistory"
    ]:
        value = values.get(field_name)
        if value:
            try:
                ar.getField(field_name).set(ar, value)
            except Exception as e:
                logger.warn("写入字段 %s 出错: %s", field_name, e)
