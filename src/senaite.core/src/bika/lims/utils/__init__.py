...
def get_subject_link(context, subject_uid, value=None, **kwargs):
    if not subject_uid:
        return ""

    suid = api.safe_unicode(subject_uid).strip()
    if not suid:
        return ""

    portal = api.get_portal()   # 使用跟站点
    base = api.get_url(portal).rstrip("/")    # http://localhost:8080/TCRx
    href = u"{}/@@subject?uid={}".format(base, quote(suid.encode("utf-8")))
...
def get_project_link(context, project, title=None, base_path="samples/project-center-samples",
                     review_state="all", extra=None, **kwargs):

    project = api.safe_unicode(project or u"").strip()
    if not project:
        return u""

    text = api.safe_unicode(title or project)

    portal = api.get_portal()
    base = api.get_url(portal).rstrip("/")
    href = u"{}/{}".format(base, base_path.lstrip("/"))

    params = {"project": project}
    if review_state:
        params["samples_review_state"] = review_state
    if extra and isinstance(extra, dict):
        params.update(extra)

    def _q(v):
        return quote(api.safe_unicode(v).encode("utf-8"), safe="")

    pairs = ["%s=%s" % (_q(k), _q(v)) for k, v in params.items()]
    qs = "&".join(pairs)
    href = u"{}?{}".format(href, api.safe_unicode(qs))

    kwargs.setdefault("csrf", False)
    return get_link(href=href, value=text, **kwargs)

def get_samples_filter_link(text, context, filter_kind, filter_value,
                            filter_label=None, project=None,
                            client_uid=None, client_id=None):
    """Return anchor html for samples filter page"""
    text = safe_unicode(text or u"")
    filter_kind = safe_unicode(filter_kind or u"")
    filter_value = safe_unicode(filter_value or u"")
    filter_label = safe_unicode(filter_label or text or filter_value)

    portal = context.portal_url.getPortalObject()
    base = portal.absolute_url().rstrip("/") + "/projects/project_center_samples_filter"

    params = [
        "filter_kind=%s" % quote(filter_kind.encode("utf-8")),
        "filter_value=%s" % quote(filter_value.encode("utf-8")),
        "filter_label=%s" % quote(filter_label.encode("utf-8")),
        "samples_review_state=all",
    ]

    if project:
        params.append("project=%s" % quote(safe_unicode(project).encode("utf-8")))
    if client_uid:
        params.append("client_uid=%s" % quote(safe_unicode(client_uid).encode("utf-8")))
    if client_id:
        params.append("client_id=%s" % quote(safe_unicode(client_id).encode("utf-8")))
