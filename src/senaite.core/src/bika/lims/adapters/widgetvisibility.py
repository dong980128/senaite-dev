...
class SampleFieldsAfterReceivedVisibility(SenaiteATWidgetVisibility):
    def __init__(self, context):
        super(SampleFieldsAfterReceivedVisibility, self).__init__(
            context=context,
            # 提高优先级，先于其它规则运行
            sort=1,
            field_names=[
                "ThermometerStatus", "BoxPreCooled", "SampleIsolated", "BoxPadded",
                "TemperatureRecorded", "TransportMethod", "Shipper", "ShippingTime",
                "StorageConditions", "ThermometerCode",
                "over24h", "labelMatch", "pkgIntact", "tempValid", "pkgStatus", "pkgStatusNote",
                # 补充到受控字段列表，否则这个适配器不会被调用
                "TrueDateReceived",
            ],
        )

    def isVisible(self, field, mode="edit", default="visible"):
        field_name = field.getName()
        mt = getToolByName(self.context, "portal_membership")
        member = mt.getAuthenticatedMember()
        roles = set(member.getRolesInContext(self.context))  # 关键：转成 set
        ...
        in_group = field_name in group1_fields or field_name in group2_fields
        both_roles = {"Analyst", "LabClerk"}.issubset(roles)

        if in_group and both_roles:
            return "invisible"

        if field_name in group1_fields:
            allowed_roles = {"LabClerk", "Sampler", "Manager"}
        elif field_name in group2_fields:
            allowed_roles = {"LabClerk", "Manager"}
        else:
            return default

        # 命中任一允许角色 -> 按默认显示，否则不可见
        return default if (roles & allowed_roles) else "invisible"

class ProfilesFieldVisibility(SenaiteATWidgetVisibility):

    def __init__(self, context):
        super(ProfilesFieldVisibility, self).__init__(
            context=context, sort=10, field_names=["Profiles"])

    def isVisible(self, field, mode="view", default="visible"):
        allowed_roles = {"Manager", "LabManager", "Analyst"}

        mt = getToolByName(self.context, "portal_membership")
        member = mt.getAuthenticatedMember()
        roles = set(member.getRolesInContext(self.context))
