...
logger = logging.getLogger("senaite.analysislog")
...
from bika.lims.workflow.pipeline import pipeline_blocks_ar_transition, find_taskrun_for_analysis
...
def after_submit(analysis):
    ...
    if IRequestAnalysis.providedBy(analysis):
        ...
        # Gate: if pipeline (LabProcessRun) is running and not finished, do NOT
        # promote AR workflow yet (prevents AR going to to_be_verified too early).

        if pipeline_blocks_ar_transition(ar, require_modes=("analysis",)):
            logger.info(
                "[WF][GATE] Block AR submit because pipeline not finished. AR=%s analysis=%s",
                api.get_id(ar), api.get_id(analysis)
            )
            reindex_request(analysis)
            return
...
def after_retract(analysis):
    ...
    # ===== Default behavior (non-pipeline analyses) =====
    if IRequestAnalysis.providedBy(analysis):
        doActionFor(analysis.getRequest(), "rollback_to_receive")
        reindex_request(analysis)
