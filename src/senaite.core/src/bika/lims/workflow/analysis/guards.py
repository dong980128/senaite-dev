...
# the terms of the GNU General Public License as published by the Free SoftwareR
...
from lims.workflow.analysis.test import _get_department_manager_userid, user_is_site_manager
...
def guard_submit(analysis):
    ...
    Only the assigned analyst is allowed to submit
    """

    current_user = security.get_user_id()
    assigned_analyst = analysis.getAnalyst()
    ...
    # Cannot submit if attachment required but not provided
    if not analysis.getAttachment() and analysis.getAttachmentRequired():
        return False

    # Check if Sample is received or sampled
    if IRequestAnalysis.providedBy(analysis):
        point_of_capture = analysis.getPointOfCapture()
        if point_of_capture == "lab" and not analysis.isSampleReceived():
            logger.info("guard_submit: sample not received")
            return False
        if point_of_capture == "field" and not analysis.isSampleSampled():
            logger.info("guard_submit: sample not sampled")
            return False

    # Enforce assigned analyst restriction unless override is allowed
    if not analysis.bika_setup.getAllowToSubmitNotAssigned():
        if not user_has_super_roles():
            if not assigned_analyst:
                logger.info("guard_submit: no assigned analyst")
                return False
            if assigned_analyst != current_user:
                logger.info("guard_submit: current user is not assigned analyst")
                return False

    # Ensure all dependencies are submitted or submittable
    for dependency in analysis.getDependencies():
        if not is_submitted_or_submittable(dependency):
            logger.info("guard_submit: dependency not ready: %s", dependency)
            return False
...
def guard_verify(analysis):
    ...
    """
    # Cannot verify i the number of remaining verifications is > 1

    current_user = security.get_user_id()
    is_site_manager = user_is_site_manager()

    if is_site_manager:
        pass

    else:
        # 非站点 Manager：只能审核自己部门
        manager_id = _get_department_manager_userid(analysis)

        if not manager_id:
            # 这个服务/部门没配置 Manager，普通用户一律不允许审核
            logger.warning(
                u"guard_verify: no department manager configured, denying verify for user %s",
                current_user,
            )
            return False
        ...
        if current_user != manager_id:
            logger.warning(
                u"guard_verify: user %s is not department manager %s, denying verify",
...
def guard_retract(analysis):
    ...
    """
    current_user = security.get_user_id()
    assigned_analyst = getattr(analysis, "Analyst", None)  # 字段名注意首字母大写

    if assigned_analyst != current_user and not user_has_super_roles():
        return False
