...
from bika.lims.workflow.analysisrequest.autoassign import auto_assign_analyst
...
def after_receive(analysis_request):
    ...
    now = DateTime().toZone("Asia/Shanghai")
    analysis_request.setDateReceived(now)

    do_action_to_analyses(analysis_request, "initialize")
    auto_assign_analyst(analysis_request)
