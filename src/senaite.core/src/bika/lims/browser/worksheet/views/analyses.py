...
class AnalysesView(BaseView):
    ...
    def before_render(self):
        ...
        self.enable_create_worksheet()

    def enable_create_worksheet(self):
        """在工作表右键/批量菜单里加入：创建工作表"""
        portal = api.get_portal()
        url = portal.absolute_url() + "/samples/create_worksheet_modal"
        ...
        for state in self.review_states:
            ct = state.setdefault("custom_transitions", [])
            # 防重复注入
            if any(x.get("id") == action["id"] for x in ct):
                continue
            ct.append(action)
    ...
    def folderitems(self):
        ...
        self._recompute_items_rowspans(items)
        self._debug_dump_slots(items, tag="after_base")
        ...
        self._debug_dump_slots(items, tag="after_sort")
    ...
    def _debug_dump_slots(self, items, tag=""):
        ...
        for it in items:
            ...
            by_slot[pos].append({
                ...
                "pos_sortkey": it.get("pos_sortkey", ""),
                "skip": it.get("skip", []),
            })

        # for pos in sorted([k for k in by_slot.keys() if isinstance(k, int)]):
        #     arr = by_slot[pos]
        #     for i, x in enumerate(arr, 1):
        #         logger.info("[WS][%s]   #%d uid=%s disabled=%s skip=%s service=%s sort=%s",
        #                     tag, i, x["uid"], x["disabled"], x["skip"], x["service"], x["pos_sortkey"])

    def _recompute_items_rowspans(self, items):
        """Recompute rowspans based on FINAL items (after security/visibility filtering)."""
        self.items_rowspans = {}

        # 1) 先按 slot 统计“实际会渲染出来的 analysis 行数”
        for it in items:
            ...
            if not isinstance(pos, int):
                continue
            self.items_rowspans[pos] = self.items_rowspans.get(pos, 0) + 1

        #    则 header 需要 rowspan = count * 2
        if self.is_analysis_remarks_enabled():
            for pos, cnt in list(self.items_rowspans.items()):
                self.items_rowspans[pos] = cnt * 2
