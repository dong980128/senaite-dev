...
import logging
logger = logging.getLogger("senaite.worksheet")
...
class AddAnalysesView(BikaListingView):
    ...
    def folderitem(self, obj, item, index):
        ...
        try:
            # 获取 AnalysisRequest 对象
            ar = obj.getAnalysisRequest() if hasattr(obj, "getAnalysisRequest") else api.get_parent(obj)
            if ar:
                subject_uid = ar.getSubjectUID() if hasattr(ar, "getSubjectUID") else None
                item["SubjectUID"] = subject_uid or ""
                logger.info("[AddAnalysesView] 获取样本 SubjectUID: %s", subject_uid)
            else:
                logger.warning("[AddAnalysesView] 无法找到样本对象 for analysis: %s", obj.getId())
        except Exception as e:
            logger.error("[AddAnalysesView] 获取 SubjectUID 异常: %s", str(e))
