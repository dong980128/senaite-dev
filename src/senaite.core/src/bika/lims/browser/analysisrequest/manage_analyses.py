...
class AnalysisRequestAnalysesView(BikaListingView):
    ...
    def folderitems(self):
        ...
        allowed_keywords = self.get_allowed_keywords()
        ...
        def is_allowed(item):
            ...
            try:
                obj = api.get_object(obj)  # 确保 brain 转对象
                kw = obj.getKeyword()
            except Exception:
                return False
            ...
            return kw in allowed_keywords

        filtered = [it for it in items if is_allowed(it)]
        return filtered
    ...
    def folderitem(self, obj, item, index):
        ...
        if uid in self.analyses:
            ...
            if review_state not in DETACHED_STATES:
                ...
                # 判断当前用户不是assigned_to的分析员，则禁用
                current_user = api.get_current_user()
                assigned_to = analysis.getAnalyst()
    ...
    def get_allowed_keywords(self):
        """返回当前用户允许的 AnalysisService keyword 列表
        - admin 用户：返回 "__ALL__" 表示放行所有服务
        - 普通用户：返回绑定的服务关键字列表
        - 无联系人或无绑定服务：返回 []
        """
        ...
        try:
            ...
            for brain in brains:
                try:
                    obj = brain.getObject()
                    if obj.getUsername() == user_id:
                        contact = obj
                        break
                except Exception as e:
                    logger.warning("获取联系人对象失败：%s", e)
        except Exception as e:
            logger.warning("catalog 异常：%s", e)

        # fallback：容器遍历查找
        if not contact:
            try:
                labcontacts = getToolByName(self.context, "bika_setup").bika_labcontacts
                for obj in labcontacts.objectValues():
                    if obj.getUsername() == user_id:
                        contact = obj
                        break
            except Exception as e:
                logger.warning("fallback 遍历联系人失败：%s", e)

        if not contact:
            logger.warning("无法获取联系人，终止权限过滤")
            return []

        # 获取联系人绑定的服务 keyword 列表
        try:
            services = contact.getAllowedServices() or []
        except Exception as e:
            logger.warning("读取 AllowedServices 失败：%s", e)
            return []
        ...
        for svc in services:
            ...
            try:
                ...
                logger.warning("获取服务 keyword 出错：%s", e)
