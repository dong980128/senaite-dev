...
class EmailView(BrowserView):
    ...
    def max_email_size(self):
        ...
        reg = api.get_registry_record("senaite.core.max_email_size")
        max_size = DEFAULT_MAX_EMAIL_SIZE if reg is None else reg
        if max_size < 0:
            max_size = 0
        return max_size * 1024  # KB
        ...
        for num, report in enumerate(reports):
            ...
            for recipient in self.get_recipients(sample):
                ...
                record = {
                    ...
                    "email": email,
                    "address": address,
                    "valid": True,  # 仍按“是否所有报告都有”去标记
                    "is_primary": rtype == "to",  # 主联系人标记
                    "checked": False,
                }
    ...
    def get_recipients(self, ar):
        ...
        def recipient_from_contact(contact, rtype="cc_contact"):
            if not contact:
                return None
        ...
        to = filter(None, [recipient_from_contact(ar.getContact(), rtype="to")])
        # CC Contacts
        cc = filter(None, map(lambda c: recipient_from_contact(c, "cc_contact"), ar.getCCContact()))
        # CC Emails
    ...
    def ajax_user_emails(self, *path_args, **path_kwargs):
        """POST /.../email/user_emails
        返回: {"ok": True, "items": [{"name":"", "email":"", "address":"Name <email>"}]}
        搜索顺序（直到凑满 limit 为止）：
          1) 当前 Client 下的联系人
          2) Setup（实验室联系方式）下的联系人
          3) 全站联系人（不限制路径）
          4) 站点用户（Members）
        """
        req = self.request
        term = api.safe_unicode(req.get("term", u"")).strip()
        try:
        ...
        def _is_email(s):
            return re.match(r"^[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}$", s or u"") is not None

        def _fmt(name, email):
            name = api.safe_unicode(name or u"")
            email = api.safe_unicode(email or u"")
            return u"%s <%s>" % (name, email) if name else email

        def _norm(s):
            # 去掉所有空白（含全角空格）并小写
            return re.sub(ur'[\s\u3000]+', u'', api.safe_unicode(s or u'')).lower()

        def _add_item(items, seen, name_u, email_u):
            key = (email_u or u"").lower()
            if not key or key in seen:
                return False
            seen.add(key)
            items.append({"name": name_u or u"", "email": email_u, "address": _fmt(name_u, email_u)})
            return True
        ...
        def _get_client():
            ...
            try:
                from bika.lims.interfaces import IAnalysisRequest
                if IAnalysisRequest.providedBy(ctx):
                    return ctx.getClient()
                if getattr(ctx, "portal_type", "") == "Client":
                    return ctx
                if hasattr(ctx, "getAnalysisRequest"):
                    ar = ctx.getAnalysisRequest()
                    if ar:
                        return ar.getClient()
            except Exception:
                return None
        ...
        def _collect_from_contacts(path_filter, stage_name):
            """在指定 path（或 None=全站）收集联系人
               依次尝试 portal_catalog / bika_setup_catalog / bika_catalog；
               若仍为空且给了 path，则遍历对象兜底。
            """
            target_types = ("Contact", "LabContact", "Person", "ClientContact", "ARContact")
            results, seen_rids = [], set()

            def _brains_from_catalog(catalog_id, query):
                try:
            ...
            # 1) 多 catalog 检索
            query = {"portal_type": list(target_types)}
            if path_filter:
                query["path"] = {"query": path_filter}

            for cat_id in ("portal_catalog", "bika_setup_catalog", "bika_catalog"):
                brains = _brains_from_catalog(cat_id, query)
                for b in brains:
                    rid = getattr(b, "getRID", lambda: None)()
                    if rid in seen_rids:
                        continue
                    seen_rids.add(rid)
                    results.append(b)
            ...
            # 2) 遍历兜底
            if not results and path_filter:
                try:
                    root = api.get_portal()
                    container = root.unrestrictedTraverse(path_filter, default=None)

                    def _walk(obj):
                        try:
                            children = list(obj.objectValues()) if hasattr(obj, "objectValues") else []
                        except Exception:
                            children = []
                        for child in children:
                            pt = getattr(child, "portal_type", "")
                            if pt in target_types:
                                class _B(object):
                                    ...
                                    def getRID(self): return id(self._o)

                                results.append(_B(child))
                            _walk(child)
            ...
            for b in results:
                ...
                try:
                    ...
                    # 邮箱
                    email = None
                    for getter in ("getEmailAddress", "getEmail", "email"):
                        if hasattr(obj, getter):
                            v = getattr(obj, getter)
                            email = v() if callable(v) else v
                            if email:
                                break
                    if not email:
                        continue
                    ...
                    # 姓名
                    name = None
                    for getter in ("Title", "getFullname", "getFullName", "getName"):
                        if hasattr(obj, getter):
                            v = getattr(obj, getter)
                    ...
                    # 用户名 / Linked System User / 对象ID
                    try:
                        username = obj.getUsername() or ""
                    except Exception:
                        username = ""
                    linked = ""
                    for getter in ("getLinkedSystemUser", "getLinkedUser", "getPrimaryUser", "getUserId"):
                        if hasattr(obj, getter):
                            v = getattr(obj, getter)
                            linked = v() if callable(v) else v
                            if linked:
                                break
                    cid = api.get_id(obj) or ""

                    email_u = api.safe_unicode(email or u"")
                    name_u = api.safe_unicode(name or u"")
                    username_u = api.safe_unicode(username or u"")
                    linked_u = api.safe_unicode(linked or u"")
                    cid_u = api.safe_unicode(cid or u"")

                    hay_raw = u" ".join([email_u, name_u, username_u, linked_u, cid_u])
                    if term and _norm(term) not in _norm(hay_raw):
                        continue

                    _add_item(items, seen, name_u, email_u)
        ...
        if client is not None:
            _collect_from_contacts("/".join(client.getPhysicalPath()), stage_name="client")

        # 2) Setup
        if len(items) < limit and setup is not None:
            _collect_from_contacts("/".join(setup.getPhysicalPath()), stage_name="setup")

        # 3) 全站
        if len(items) < limit:
            _collect_from_contacts(None, stage_name="site")
        ...
        if len(items) < limit:
            ...
            for m in members:
                if len(items) >= limit:
                    break
                try:
                    email_u = api.safe_unicode(m.getProperty("email", "") or u"")
                    if not email_u:
                        continue
                    name_u = api.safe_unicode(m.getProperty("fullname", "") or m.getId() or u"")
                    username_u = api.safe_unicode(m.getId() or u"")
                    hay_raw = u" ".join([email_u, name_u, username_u])
                    if term and _norm(term) not in _norm(hay_raw):
                        continue
                    _add_item(items, seen, name_u, email_u)
                except Exception:
        ...
        # 直接输入是邮箱 -> 放首位
        if term and _is_email(term) and term.lower() not in seen:
            items.insert(0, {"name": u"", "email": term, "address": term})
