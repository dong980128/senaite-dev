...
def to_localized_time(dt, long_format=None, time_only=None,
    ...
    if not dt:
        return default
    # logger.info("[DEBUG] to_localized_time() 原始类型：%s，值：%s", type(dt), dt)

    # 强制去除时区
    if hasattr(dt, 'asdatetime'):
        dt = dt.asdatetime().replace(tzinfo=None)
        # logger.info("[DEBUG] asdatetime() 后去 tzinfo：%s", dt)
    elif hasattr(dt, 'tzinfo') and dt.tzinfo:
        # logger.info("[DEBUG] datetime 带 tzinfo：%s", dt.tzinfo)
        dt = dt.replace(tzinfo=None)
        # logger.info("[DEBUG] replace() 去 tzinfo 后：%s", dt)
    else:
        logger.warn("[DEBUG] 无需处理 tzinfo：%s", dt)

    try:
        ts = get_tool("translation_service")
        time_str = ts.ulocalized_time(
            dt, long_format, time_only, context, "senaite.core", request)
        # logger.info("[DEBUG] 翻译服务输出：%s", time_str)
    except ValueError:
        logger.warn("[WARN] ulocalized_time() 报错，进入兼容模式")
        # Handle dates < 1900
        ...
        # logger.info("[DEBUG] 使用 translate 获取 formatstring=%s", formatstring)
        ...
        logger.info("[DEBUG] 最终返回字符串：%s", time_str)
