...
class ILabProcessRegistry(ISenaiteRegistry):
    """LabProcess settings"""

    model.fieldset(
        "labprocess",
        label=_(u"LabProcess"),
        description=_("LabProcess configuration"),
        fields=[
            "labprocess_config_json",
        ],
    )

    # 用 schema.Text：允许中文 + 长 JSON（别用 ASCIILine）
    labprocess_config_json = schema.Text(
        title=_(u"LabProcess config (JSON)"),
        required=False,
        default=u"",
    )
