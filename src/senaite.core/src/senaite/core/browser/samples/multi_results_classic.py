...
class MultiResultsClassicView(BrowserView):
    ...
    def show_labprocess_runs(self, sample):
        """判断该 sample 是否有 LabProcessRun，决定是否显示实验流程 tab"""
        try:
            pc = api.get_tool("portal_catalog")
            ar_path = "/".join(sample.getPhysicalPath())
            brains = pc(
            ...
            logger.exception(
                "[MultiResultsClassicView] show_labprocess_runs failed sample=%s",
                api.get_id(sample)
            )
            return False

    def render_labprocess_runs(self, sample):
        """渲染单个 sample 的 LabProcessRun viewlet HTML"""
        try:
            from senaite.core.labprocess.browser.viewlets.labprocessruns import LabProcessRunsViewlet
            from Products.Five.browser.pagetemplatefile import ViewPageTemplateFile
            import os
            import senaite.core.labprocess.browser as _browser_pkg

            # 模板在 senaite.core.labprocess.browser/templates/ 下
            base_dir = os.path.dirname(os.path.abspath(_browser_pkg.__file__))
            tpl_path = os.path.join(base_dir, "templates", "labprocessruns_viewlet.pt")

            if not os.path.exists(tpl_path):
                logger.error(
                    "[MultiResultsClassicView] template not found: %s", tpl_path
                )
                return u""

            viewlet = LabProcessRunsViewlet(
                context=sample,
                request=self.request,
                view=self,
                manager=None,
            )
            viewlet.index = ViewPageTemplateFile(tpl_path).__get__(
                viewlet, type(viewlet)
            )
            viewlet.update()
            html = viewlet.render() or u""
            logger.info(
                "[MultiResultsClassicView] render_labprocess_runs sample=%s html_len=%s",
                api.get_id(sample), len(html)
            )
            return html
        except Exception:
            logger.exception(
                "[MultiResultsClassicView] render_labprocess_runs failed sample=%s",
                api.get_id(sample)
            )
            return u""
