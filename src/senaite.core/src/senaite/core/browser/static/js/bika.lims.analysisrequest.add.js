(function () {
    ...
    */
    ...
    // 生成当前列的单元格选择器
    cellSel = function (name, i) {
        return `[fieldName='${name}-${i}']`;
    };
    ...
    window.AnalysisRequestAdd = class AnalysisRequestAdd {
        constructor() {
            ...
            // —— Ethnicity 多级就地输入：初始化整表 ——
            this.init_ethnicity_inline = this.init_ethnicity_inline.bind(this);
            // 确保当前列 Ethnicity 单元格里有一个隐藏的输入框
            this.$ensure_eth_inline = this.$ensure_eth_inline.bind(this);
            // 绑定下拉改变事件
            this.$bind_ethnicity_change = this.$bind_ethnicity_change.bind(this);
            // 根据选择项显示/隐藏右侧输入框
            this.$update_eth_inline = this.$update_eth_inline.bind(this);
            this.prepare_ethnicity_values = this.prepare_ethnicity_values.bind(this);
            this.bind_sample_type_listener = this.bind_sample_type_listener.bind(this);
            // 根据当前样本类型，显示/隐藏病灶两个字段
            this.toggle_disease_fields = this.toggle_disease_fields.bind(this);

            this.bind_thermometer_listener = this.bind_thermometer_listener.bind(this);
            this.toggle_thermometer_fields = this.toggle_thermometer_fields.bind(this);

            // 输血史联动
            this.bind_blood_transfusion_listener = this.bind_blood_transfusion_listener.bind(this);
            this.toggle_blood_transfusion_history = this.toggle_blood_transfusion_history.bind(this);
        }
        ...
        load() {
            ...
            this.init_ethnicity_inline();
            // this.bind_submit_eth();
            ...
            this.bind_sample_type_listener();
            this.bind_hospital_patient_id_listener();
            // 根据温度计是否有效，显示对应字段
            this.bind_thermometer_listener();
            // 根据12个月内是否输血，显示/隐藏输血史字段
            this.bind_blood_transfusion_listener();
        ...
        bind_eventhandler() {
            ...
            // queryselectwidget 类型的引用字段（如 Client）也需要监听
            $("body").on("select deselect", "textarea.queryselectwidget-value", this.on_referencefield_value_changed);
        ...
        set_reference_field_query(field, query) {
            var controller, data, me, target_base_query, target_catalog, target_field_label, target_field_name,
                target_query, target_value;
        ...
        on_referencefield_value_changed(event) {
            var $el, after_change, arnum, deselected, el, event_data, field_name, filter_queries, manually_deselected,
                me, metadata, record, ref, sampleField, selected, showForTypes, value;
            me = this;
            el = event.currentTarget;
            $el = $(el);
            ...
            if (field_name === "SampleType") {
                showForTypes = ['新鲜组织', '切片', '蜡块', '蜡卷'];
                var showCollectionMethodTypes = ['新鲜组织'];
                sampleField = document.querySelector(`#SampleType-${arnum}`);
                if (sampleField != null) {
                    setTimeout((() => {
                        this.toggle_disease_fields(sampleField, showForTypes);
                    }), 0);
                }
            }

            // Client字段的 queryselectwidget 事件 bubbles:false，
            // 改用 bind_hospital_patient_id_listener 里的 MutationObserver 处理
        ...
        on_copy_click(event) {
            ...
            if ($(td1).find('.ArchetypesReferenceWidget').length > 0) {
                ...
                $.each((function () {
                    var results = [];
                    for (var j = 1; 1 <= ar_count ? j <= ar_count : j >= ar_count; 1 <= ar_count ? j++ : j--) {
                        results.push(j);
                    }
                    return results;
        ...
        on_form_submit(event, callback) {
            ...
            event.preventDefault();
            me = this;
            if (typeof me.prepare_ethnicity_values === "function") {
                me.prepare_ethnicity_values();
            }
        ...
        file_addbtn_click(event, element) {
            var arnum, counter, del_btn, del_btn_src, existing_file_field_names, existing_file_fields, file_field,
                file_field_div, holding_div, name, newfieldname;
        ...
        init_ethnicity_inline() {
            var arCount, i, j, ref, results;
            arCount = parseInt($('#ar_count').val() || '1', 10);
            results = [];
            for (i = j = 0, ref = arCount; (0 <= ref ? j < ref : j > ref); i = 0 <= ref ? ++j : --j) {
                this.$ensure_eth_inline(i);
                this.$bind_ethnicity_change(i);
                results.push(this.$update_eth_inline(i));
            }
            return results;
        }
        ...
        $ensure_eth_inline(i) {
            var $cell, $input, $wrap;
            $cell = $(cellSel('Ethnicity', i));
            if (!$cell.length) {
                return;
            }
            if ($cell.find('.eth-inline').length) {
                return;
            }
            $wrap = $('<span class="eth-inline" style="margin-left:8px;"></span>');
            $input = $('<input type="text" class="form-control form-control-sm" ' + 'style="display:none;width:160px;vertical-align:middle;" ' + 'placeholder="国籍">');
            $wrap.append($input);
            return $cell.append($wrap);
        }

        $bind_ethnicity_change(i) {
            var $sel;
            $sel = $(cellSel('Ethnicity', i)).find('select,input');
            return $sel.off('change.eth').on('change.eth', () => {
                return this.$update_eth_inline(i);
            });
        }

        $update_eth_inline(i) {
            var $cell, $input, $sel, SEP, country, parts, val;
            SEP = '|';
            $cell = $(cellSel('Ethnicity', i));
            if (!$cell.length) {
                return;
            }
            $sel = $cell.find('select,input');
            val = ($sel.val() || '').trim();
            $input = $cell.find('.eth-inline input');
            // 兼容已存在的组合值：拆回“Foreigner + 国家”，并回显到输入框
            if (val.indexOf(`Foreigner${SEP}`) === 0) {
                parts = val.split(SEP);
                country = (parts[1] || '').trim();
                $sel.val('Foreigner');
                $input.val(country).show();
                return;
            }
            // 常规联动
            if (val === 'Foreigner') {
                ...
                return $input.val('').hide();
            }
        }

        prepare_ethnicity_values() {
            var $cell, $input, $sel, arCount, country, i, j, ref, val;
            arCount = parseInt($('#ar_count').val() || '1', 10);
            for (i = j = 0, ref = arCount; (0 <= ref ? j < ref : j > ref); i = 0 <= ref ? ++j : --j) {
                $cell = $(cellSel('Ethnicity', i));
                if (!$cell.length) {
                ...
                $input = $cell.find('.eth-inline input');
                if (val === 'Foreigner') {
                    country = ($input.val() || '').trim();
                    if (country) {
                        if ($sel.is('select') && !$sel.find(`option[value='${country}']`).length) {
                            $sel.append($('<option/>').val(country).text(country));
                        }
                        $sel.val(country);
                    } else {
                        $sel.val('Foreigner');
        ...
        bind_sample_type_listener() {
            var showForTypes;
            showForTypes = ['新鲜组织', '切片', '蜡块', '蜡卷'];
            var showCollectionMethodTypes = ['新鲜组织'];
            return document.querySelectorAll('[id^="SampleType"]').forEach((sampleField) => {
                return setTimeout((() => {
                    this.toggle_disease_fields(sampleField, showForTypes);
                }), 200);
            });
        ...
        toggle_disease_fields(sampleField, showForTypes) {
            let raw = "";
            let selectedValue = "";
            ...
            const widgetId = sampleField.id + "-widget";
            let widget = document.getElementById(widgetId);

            if (!widget) {
                widget = sampleField.closest(".queryselectwidget") ||
                    sampleField.closest(".uidreferencewidget") ||
                    sampleField.parentElement;
            }

            let labelNode =
                widget && (
                    widget.querySelector(".queryselectitem") ||
                    widget.querySelector(".queryselectwidget-items") ||
                    widget.querySelector("span")
                );
            if (labelNode) {
                raw = (labelNode.textContent || "").trim();
            }
            const match = raw.match(/^[\u4e00-\u9fa5]+/);
            selectedValue = match ? match[0] : "";
            const shouldShow =
                !!selectedValue && showForTypes.indexOf(selectedValue) !== -1;
            const row1 = document.querySelector('tr[fieldname="LesionType"]');
            const row2 = document.querySelector('tr[fieldname="LesionDescription"]');

            if (row1) row1.style.display = shouldShow ? "" : "none";
            if (row2) row2.style.display = shouldShow ? "" : "none";

            // 采集方式：仅新鲜组织显示
            const rowCollection = document.querySelector('tr[fieldname="CollectionMethod"]');
            if (rowCollection) {
                rowCollection.style.display = selectedValue === '新鲜组织' ? "" : "none";
            }
        }
        ...
        bind_hospital_patient_id_listener() {
            // MutationObserver直接监听Client字段行的DOM变化
            const getClientUID = (idx) => {
                const clientTextarea =
                    document.querySelector(`tr[fieldname="Client-${idx}"] textarea.queryselectwidget-value`) ||
                    document.querySelector(`tr[fieldname="Client"] textarea.queryselectwidget-value`);
                if (clientTextarea && clientTextarea.value) {
                    return clientTextarea.value.trim();
                }

                const clientHidden = document.querySelector(`input[name="Client-${idx}"]`);
            ...
            const checkAndToggle = (idx) => {
                const uid = getClientUID(idx);
                this.toggle_hospital_patient_id(uid, !!uid, idx);
            };

            const arCount = parseInt($("#ar_count").val() || "1", 10);

            for (let i = 0; i < arCount; i++) {
                const idx = i;
                this.toggle_hospital_patient_id("", false, idx);
                setTimeout(() => checkAndToggle(idx), 800);
                const clientRow =
                    document.querySelector(`tr[fieldname="Client-${idx}"]`) ||
                    document.querySelector(`tr[fieldname="Client"]`);

                if (clientRow) {
                    const observer = new MutationObserver(() => checkAndToggle(idx));
                    observer.observe(clientRow, {
                        childList: true,
                        subtree: true,
                        characterData: true,
                        attributes: true
        ...
        toggle_hospital_patient_id(clientUID, selected, arnum) {
            const row = document.querySelector(`tr[fieldname="HospitalPatientID"]`);
            if (!row) return;
            const shouldShow = selected && this.HOSPITAL_CLIENT_UIDS.indexOf(clientUID) !== -1;
            row.style.display = shouldShow ? "" : "none";
            if (!shouldShow) {
                const input = row.querySelector(`input[name^="HospitalPatientID"]`);
                if (input) input.value = "";
            }
        }

        // 是否涉及温度运输的选择
        bind_thermometer_listener() {
            const arCount = parseInt($("#ar_count").val() || "1", 10);

            for (let i = 0; i < arCount; i++) {
                const field = document.querySelector(`#TempControlledTransport-${i}`);
                if (!field) continue;
                this.toggle_thermometer_fields(field);
                field.addEventListener("change", (ev) => {
                    this.toggle_thermometer_fields(ev.target);
                });
            }
        }

        toggle_thermometer_fields = (statusField) => {
            if (!statusField) return;

            const rawValue = (statusField.value || "").trim().toLowerCase();
            const shouldShow = rawValue === "yes";

            const controlledFieldnames = [
                "ThermometerStatus",
                "ThermometerCode",
                "TemperatureRecorded",
                "BoxPreCooled",
            ...
            controlledFieldnames.forEach((fname) => {
                const row = document.querySelector(`tr[fieldname="${fname}"]`);
                if (!row) return;
        ...
        // 根据"12个月内是否输血"显示/隐藏输血史字段
        bind_blood_transfusion_listener() {
            const arCount = parseInt($("#ar_count").val() || "1", 10);

            for (let i = 0; i < arCount; i++) {
                const field = document.querySelector(`#BloodTransfusion12M-${i}`);
                if (!field) continue;

                // 初始化：根据当前值决定是否显示
                this.toggle_blood_transfusion_history(field);

                // 监听变更
                field.addEventListener("change", (ev) => {
                    this.toggle_blood_transfusion_history(ev.target);
                });
            }
        }

        toggle_blood_transfusion_history = (selectField) => {
            if (!selectField) return;

            const rawValue = (selectField.value || "").trim().toLowerCase();
            const shouldShow = rawValue === "yes";

            const row = document.querySelector(`tr[fieldname="BloodTransfusionHistory"]`);
            if (!row) return;
