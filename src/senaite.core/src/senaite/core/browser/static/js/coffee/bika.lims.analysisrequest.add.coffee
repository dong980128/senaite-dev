...
# 生成当前列的单元格选择器
cellSel = (name, i) -> "[fieldName='#{name}-#{i}']"
...
class window.AnalysisRequestAdd
  ...
  on_referencefield_value_changed: (event) =>
    ...
    if field_name is "SampleType"
      showForTypes = ['新鲜组织', '切片', '蜡块', '蜡卷']
      sampleField = document.querySelector "#SampleType-#{arnum}"
      if sampleField?
        setTimeout (=> @toggle_disease_fields(sampleField, showForTypes)), 0
  ...
  init_ethnicity_inline: =>
    arCount = parseInt($('#ar_count').val() or '1', 10)
    for i in [0...arCount]
      @$ensure_eth_inline(i)
  ...
  $bind_ethnicity_change: (i) =>
    ...
    $sel.off('change.eth').on 'change.eth', => @$update_eth_inline(i)
  ...
  $update_eth_inline: (i) =>
    ...
    # 1) 兼容已存在的组合值：拆回“Foreigner + 国家”，并回显到输入框
    if val.indexOf("Foreigner#{SEP}") is 0
      parts = val.split(SEP)
      country = (parts[1] or '').trim()
      $sel.val('Foreigner')        # 下拉显示 Foreigner
      $input.val(country).show()   # 输入框显示国家
      return
  ...
  prepare_ethnicity_values: =>
      ...
      for i in [0...arCount]
        ...
        $input = $cell.find('.eth-inline input')
        if val is 'Foreigner'
          country = ($input.val() or '').trim()

          if country
            if $sel.is('select') and not $sel.find("option[value='#{country}']").length
  ...
  bind_sample_type_listener: =>
    showForTypes = ['新鲜组织', '切片', '蜡块', '蜡卷']
    document.querySelectorAll('[id^="SampleType"]').forEach (sampleField) =>
      setTimeout (=> @toggle_disease_fields(sampleField, showForTypes)), 200

  # 根据当前样本类型，显示/隐藏病灶两个字段
  toggle_disease_fields: (sampleField, showForTypes) =>
    index = sampleField?.id?.split('-')[1] or '0'

    raw = sampleField.querySelector('.queryselectwidget-selected-values')?.textContent?.trim() or ''

    labelMatch = raw.match(/^[\u4e00-\u9fa5]+/)
    selectedValue = if labelMatch then labelMatch[0] else ""

    console.log "选择样本类型为：#{if selectedValue then selectedValue else '[空]'}，索引=#{index}"

    shouldShow = selectedValue and (selectedValue in showForTypes)
    console.log "是否显示病灶字段: #{shouldShow}"

    row1 = document.querySelector 'tr[fieldname="LesionType"]'
    row2 = document.querySelector 'tr[fieldname="LesionDescription"]'

    unless row1 and row2
      console.warn '无法找到病灶字段的行！'

    if row1?
      row1.style.display = if shouldShow then '' else 'none'
    if row2?
      row2.style.display = if shouldShow then '' else 'none'
