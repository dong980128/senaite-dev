...
class IAnalysisProfileSchema(model.Schema):
    ...
    pipeline_stages = schema.Text(
        title=_(
            u"Pipeline stages configuration",
            default=u"Pipeline stages configuration",
        ),
        description=_(
            u"JSON list of stages. Example:\n"
            u"[\n"
            u"  {\"step\": 1, \"services\": [\"HLA_DNA_TYPING\", \"PBMC\"],"
            u" \"users\": [\"user_lab_1\"]},\n"
            u"  {\"step\": 2, \"services\": [\"HLA_BIOINFO\"],"
            u" \"users\": [\"user_bio_1\"]}\n"
            u"]"
        ),
        required=False,
